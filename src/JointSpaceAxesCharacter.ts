...
export class JointSpaceAxesCharacter extends AnimatedCharacter
{
    ...
    public override addGeometryToAnimatedBone(bone: AnimatedBone): void
    {
        ...
        const axesGeometry = gfx.Geometry3Factory.createAxes(size);
        axesGeometry.setLocalToParentMatrix(bone.jointSpaceToBoneSpace, false); //X,Y,Z direction of *Joint Space* (not Bone Space)

        bone.add(axesGeometry);
